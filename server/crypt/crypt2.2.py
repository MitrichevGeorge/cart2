...
class Communicator:
    ...
    def get_public_key(self):
        signature = self.sign_priv.sign(self.pub.public_bytes_raw())
        return (
    ...
    def finalize_connection(
        ...
        self.other_sign_pub = \
            CryptoUtils.deserialize_ed25519_key(
                peer_signpub_b64
            )

        peer_pub = \
            CryptoUtils.deserialize_x25519_key(
                peer_public_key_b64
            )

        if not CryptoUtils.check_sign(
            self.other_sign_pub,
            sign_public_key,
        ...
        material = CryptoUtils.derive_session_material(shared_secret, salt)
...
p1_pub, p1_sign_pub, p1_sign = peer1.get_public_key()
p2_pub, p2_sign_pub, p2_sign = peer2.get_public_key()

peer1.finalize_connection(p2_pub, p2_sign_pub, p2_sign)
peer2.finalize_connection(p1_pub, p1_sign_pub, p1_sign)
